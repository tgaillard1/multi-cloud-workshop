...
$BASE_DIR/spinnaker-for-gcp/scripts/manage/check_duplicate_dirs.sh || exit 1
...
PROPERTIES_FILE="$BASE_DIR/spinnaker-for-gcp/scripts/install/properties"
...
source $BASE_DIR/spinnaker-for-gcp/scripts/manage/service_utils.sh
...
cat > $BASE_DIR/spinnaker-for-gcp/scripts/install/properties <<EOL
#!/usr/bin/env bash
...
if [ "$SHARED_VPC_HOST_PROJECT" ]; then
  ...
  bold "  cloudshell edit $BASE_DIR/spinnaker-for-gcp/scripts/install/properties"
fi
